...
const TIER_STYLES: Record<PotentialTier, string> = {
  ...
  Legendary: 'shimmer-legendary bg-gradient-to-r from-red-900 to-orange-900 text-orange-300 border border-orange-600',
}
...
export default function PotentialBadge({ tier, size = 'md' }: Props) {
  ...
  return (
    <span className={`inline-flex rounded font-semibold ${padding} ${TIER_STYLES[tier]}`}>
      {tier}
    </span>
  )
}
